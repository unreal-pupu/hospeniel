...
export default function VatCollectedPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/earnings/vat?${params.toString()}`, {
        headers: {
      ...
      if (!response.ok) {
        console.error("Error fetching VAT:", await response.text());
        setOrders([]);
        setTotalVat(0);
        return;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">VAT Collected</h1>
          <p className="text-gray-600 mt-2">Track VAT collected on platform commissions</p>
        </div>
        <Card className="min-w-[240px]">
          <CardContent className="p-4 flex items-center gap-3">
            <Receipt className="h-6 w-6 text-hospineil-primary" />
            <div>
              <p className="text-xs text-gray-500">Total VAT Collected</p>
              <p className="text-lg font-semibold text-gray-900">
                ₦{totalVat.toLocaleString("en-NG", { minimumFractionDigits: 2 })}
              </p>
            </div>
          </CardContent>
        </Card>
                  ...
                  filteredOrders.map((order) => (
                    <tr key={order.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-gray-900">
                        ₦{(Number(order.vat_amount) || 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}
                      </td>
                      <td className="py-3 px-4">
