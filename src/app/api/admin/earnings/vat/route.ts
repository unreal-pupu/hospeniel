...
export async function GET(req: Request) {
  try {
    ...
    if (referenceList.length === 0) {
      return NextResponse.json({ orders: [], total_vat: 0 });
    }

    let ordersQuery = supabaseAdmin
      .from("orders")
      .select("id, vendor_id, user_id, vat_amount, status, created_at, payment_reference")
    ...
    const totalVat = filteredOrders.reduce(
      (sum, order) => sum + (Number(order.vat_amount) || 0),
      0
    );
